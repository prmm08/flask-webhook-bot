            ...
            for pos in r_pos["data"]:
                ...
                if key not in pos_tracker:
                    ...
                    if r_orders and isinstance(r_orders.get("data"), list):
                        ...
                        if filled:
                            pos_tracker[key] = {"level": len(filled), "last_price": float(filled[0]["avgPrice"]), "first_price": float(filled[-1]["avgPrice"])}
...
def set_tp_sl(symbol, side, current_level, first_price=None):
    ...
    api_request("POST", "/openApi/swap/v2/trade/order", {"symbol": symbol, "side": "SELL" if side == "LONG" else "BUY", "positionSide": side, "type": "TAKE_PROFIT_MARKET", "stopPrice": f"{tp_price:.6f}", "workingType": "MARK_PRICE", "closePosition": "true"})
    if USE_SL:
        sl_price = avg_price * (1 - SL_PERCENT/100) if side == "LONG" else avg_price * (1 + SL_PERCENT/100)
        api_request("POST", "/openApi/swap/v2/trade/order", {"symbol": symbol, "side": "SELL" if side == "LONG" else "BUY", "positionSide": side, "type": "STOP_MARKET", "stopPrice": f"{sl_price:.6f}", "workingType": "MARK_PRICE", "closePosition": "true"})

# --- MONITOR (DCA Check) ---
def monitor_dca():
    while not is_synced: time.sleep(2)
    while True:
        try:
            with tracker_lock:
                ...
                if r_ticker and "data" in r_ticker:
                    ...
                    if trigger and data["level"] < DCA_COUNT:
                        ...
                        if resp and resp.get("code") == 0:
                            ...
                            time.sleep(2); set_tp_sl(symbol, side, pos_tracker[key]["level"], data["first_price"])
        except: pass
        time.sleep(10)
...
def webhook():
    ...
    if currency and direction in ["LONG", "SHORT"]:
        ...
        # Strenge Prüfung: Ist IRGENDEINE Position für dieses Symbol offen?
        with tracker_lock:
        ...
        # Nur wenn das Symbol komplett neu ist, wird die Initial-Order gestartet
...
def execute_initial_trade(symbol, direction):
    ...
    if resp and resp.get("code") == 0:
        ...
        log_print(f"[SUCCESS] Erste Order für {symbol} platziert.")
        time.sleep(3)
        set_tp_sl(symbol, direction, 1, price)

@app.route("/ping")
@app.route("/")
def health(): return "BOT_V1.9.1_STRICT", 200
